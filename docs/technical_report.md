...
The dispatch-cost figures above treat over- and under-forecast symmetrically, but operationally they are not: under-forecasting means fast reserve, emergency purchase, or in the extreme, load shedding; over-forecasting only wastes committed capacity. A sensitivity sweep (`resultado_8_custo_assimetrico.png`, `reports/FACTS.md` §L) re-prices under-forecast error at 1.0×–3.0× the over-forecast rate. Chronos-2 wins at every factor tested — its margin (R\$3.01bi vs. the field's R\$7.86–8.52bi at the symmetric baseline) is too large to erode. But the ranking among the other three is **not** stable: SARIMA has the strongest directional bias toward under-forecasting (51.8% of its total absolute error comes from under-forecast hours, vs. ~50% for a well-calibrated naïve/Prophet), and by 1.5× it becomes more expensive than the naïve baseline it beats on the symmetric metric — a model can look better on a symmetric cost proxy and still be the wrong operational choice once the real asymmetry of shortage is priced in. VOLL (Value of Lost Load, ~US\$10,000/MWh in markets like MISO — orders of magnitude above typical CMO) is deliberately excluded from this sweep: it applies only to hours of actual load shedding, which this dataset does not identify (§7).
