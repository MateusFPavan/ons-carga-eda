...
**Funding.** No external funding; a personal portfolio project, with no institutional affiliation to declare.
...
- **Holiday calendar:** national holidays only, confirmed in code — `src/gerar_features.py` calls `holidays.Brazil(years=anos)` with no `subdiv` argument, which returns only the national calendar; state/municipal holidays are not encoded.
...
**Erratum policy:** because ONS revises data retrospectively, re-downloading may change historical values; any change is detectable as a hash mismatch against the manifest. **The snapshot is frozen — there is no scheduled re-pull.** This is a study with a fixed evaluation period (2024-01-01 to 2026-07-15), not a production system that needs to track a moving data source. Any future re-download would be a deliberate, new snapshot, immediately identifiable by a new hash in `MANIFEST.json` rather than a silent update to this one.
