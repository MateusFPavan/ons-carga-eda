...
- **Disaggregation:**
  ...
  - *Holiday / season / weekday breakdown* (`reports/FACTS.md` §N): Chronos-2 has the lowest MAPE in every subgroup tested (holiday vs. normal day, each season, weekday vs. weekend) — no cut changes the ranking. But its relative degradation on holidays is by far the steepest of the four: MAPE goes from 1.68% (normal days) to 7.09% (holidays), a >4× jump, versus ~1.9× for the naïve, ~1.8× for Prophet, and ~1.6× for SARIMA (the classical model degrades *least* in relative terms here). SARIMA separately shows MAPE more than doubling on weekends (4.33% → 9.08%), a pattern the other three models don't share.
  - *Asymmetric cost sensitivity* (`reports/FACTS.md` §L): re-pricing under-forecast error at 1.0×–3.0× the over-forecast rate, Chronos-2 wins at every factor tested. The ranking among the other three is not stable — SARIMA has the strongest under-forecast bias (51.8% of its absolute error) and becomes costlier than the naïve baseline once the under-forecast penalty reaches 1.5×.

## 9. Bias, risks & limitations

- **Pre-training contamination (concrete failure mode), partially — not fully — addressed:** Chronos-2 does not publish a pretraining data cutoff date, so the checkpoint's Hugging Face release date (2025-10-20) was used as a conservative proxy — any forecast origin after that date must postdate the true cutoff, whatever it is. Re-scoring all four models on just that post-cutoff window (`reports/FACTS.md` §O) still has Chronos-2 winning by a comparable margin (MASE 0.49 vs. Prophet's 1.19), which rules out the model having memorized *these specific hours* of SE/CO load. It does **not** rule out the broader risk: if Amazon's corpus included generic energy/electricity datasets with transferable load-shape structure (it does — Electricity, London Smart Meters, Buildings 900K, Solar, Wind Farms), "zero-shot" performance could still partly reflect learned analogy rather than pure generalization. Not provable from public docs; documented as an open risk.
...
- **Holiday sensitivity, quantified:** only national holidays are encoded; state/municipal holidays and atypical calendars (e.g. election days, unusual Carnaval dates) are not. Measured impact (`reports/FACTS.md` §N): Chronos-2's MAPE on holidays is 7.09%, vs. 1.68% on normal days — still the best of the four models in absolute terms, but the steepest *relative* degradation (>4×) among them.
...
`[TODO]` items flagged inline (not invented): compute footprint.
